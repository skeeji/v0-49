...
  // MODIFICATION : La logique de ce useEffect est corrigée
  useEffect(() => {
...
      if (user) {
        try {
          // On attend que les données de l'utilisateur (avec le rôle) soient bien récupérées
          await loadUserData(user.uid)
        } catch (error) {
          console.error("❌ Error loading user data:", error)
          // En cas d'erreur, on s'assure que les données utilisateur sont nulles
          setUserData(null)
        } finally {
          // MODIFICATION : On arrête le chargement seulement APRÈS que `loadUserData` soit terminé.
          setLoading(false)
        }
      } else {
        // Pas d'utilisateur connecté
        setUserData(null)
        // MODIFICATION : On arrête le chargement ici aussi pour le cas de la déconnexion.
        setLoading(false)
      }

      // MODIFICATION : L'ancien appel à setLoading(false) qui était ici a été supprimé.
    })
