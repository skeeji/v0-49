...
export default function ImportPage() {
  ...
  const handleCsvUpload = async (data: any[]) => {
    setIsUploading(true)
    try {
      // CHANGEMENT : Logique améliorée pour gérer les noms vides
      const processedData = data.map((item) => {
        const filename = item["Nom du fichier"] || "";
        const nomLuminaire = item["Nom luminaire"] || "";
        const finalNom = nomLuminaire || filename.replace(/\.[^/.]+$/, ""); // Utilise le nom de fichier si le nom est vide
      ...
      for (const luminaire of processedData) {
        // CHANGEMENT : On vérifie juste qu'il y a un nom de fichier pour la liaison
      ...
      <div className="container-responsive py-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white rounded-xl p-6 shadow-lg">
              ...
              <UploadForm
                ...
                expectedColumns={[
                  "Artiste / Dates", "Spécialité", "Collaboration / Œuvre", "Nom luminaire",
                  "Année", "Signé", "Image", "Nom du fichier", "Dimensions", "Estimation", "Matériaux",
                ]}
              />
              {csvData.length > 0 && (
          ...
          <div className="mt-8 text-center">
            <Button onClick={resetImports} variant="destructive" className="bg-red-500 hover:bg-red-600" disabled={isUploading}>
