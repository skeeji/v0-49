      ...
      <div className="min-h-screen bg-[#f5f1e8] pb-20">
        <div className="container mx-auto px-4 py-16">
          ...
          <div className="max-w-4xl mx-auto mb-12 relative">
            <div className="absolute top-[11px] left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#8b7355]/30 to-transparent" />
            <div className="flex justify-between items-start relative">
              {[
                { name: "Moyen-Age", start: 1000 },
                { name: "Renaissance", start: 1500 },
                { name: "Baroque", start: 1600 },
                { name: "Classique", start: 1700 },
                { name: "Art Nouveau", start: 1890 },
                { name: "Art Deco", start: 1920 },
                { name: "Moderne", start: 1950 },
                { name: "Contemporain", start: 2000 },
              ].map((period, index) => (
                <div key={index} className="flex flex-col items-center min-w-[70px]">
                  <div className="w-[9px] h-[9px] rounded-full bg-[#8b7355] relative z-10 ring-2 ring-[#f5f1e8]" />
                  <span className="text-[11px] font-serif text-gray-700 mt-2.5 text-center font-medium">{period.name}</span>
                  <span className="text-[10px] text-gray-400 mt-0.5">{period.start}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-12 relative">
            ...
            <div
              ref={timelineRef}
              className="flex justify-between items-start relative overflow-x-auto pb-2"
            ...
            >
              ...
              {timelineData.map((period, index) => (
                <a
                  key={period.name}
                  href={`#${period.name}`}
                  className="flex flex-col items-center group cursor-pointer flex-shrink-0 min-w-[80px]"
                >
                  <div className="w-[9px] h-[9px] rounded-full bg-[#8b7355] group-hover:scale-150 transition-transform relative z-10 ring-2 ring-[#f5f1e8]" />
                  <span className="text-[11px] font-serif text-gray-700 mt-2.5 text-center leading-tight group-hover:text-[#8b7355] transition-colors font-medium">
                    {period.name}
                  </span>
                  <span className="text-[10px] text-gray-400 mt-0.5">{period.start}</span>
                  <span className="text-[9px] text-[#8b7355]/70 mt-0.5 font-medium opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <div className="md:w-2/5 aspect-[4/3] md:aspect-auto relative bg-gray-100 overflow-hidden">
                    {period.imageUrl ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-gray-400">
                        <div className="text-6xl font-serif text-[#8b7355]/20">{period.name}</div>
                      </div>
                    )}
                    {/* Badge de la période */}
                    <div
