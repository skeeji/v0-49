...
import {
  LINKING_WORD_CAT_LABELS,
  LINKING_WORD_CAT_CONTEXT,
  LINKING_WORD_PHRASES,
  type LinkingWordCategory,
  type LinkingWordPhrase,
} from "../linking-words-data"
import { useSpeechRecognition } from "../hooks/useSpeech"
import { HoverWord } from "./HoverWord"
import { PronunciationCheck } from "./PronunciationCheck"
import type { CoachResponse } from "../types"

// Mode "Linking words" — même mécanique que NumbersSession (linking-words-data.ts
// plutôt que numbers-data.ts) : les connecteurs logiques (cause, conséquence,
// opposition...) cassent presque autant à l'oral que les chiffres, donc un
// entraînement dédié plutôt que noyés dans le vocabulaire général. Composant
// volontairement autonome, comme Chiffres, pour ne rien changer au
// fonctionnement des Flashcards existantes.

interface LinkingWord extends LinkingWordPhrase {
  mastery: number
  wrong: number
  timesShown?: number
}
...
const SERIES_SIZES = [10, 20, 32] as const
...
// autres pistes) — sans collision possible avec leurs ids ("w1", "sp-", "num-"...).
const ID_PREFIX = "lw-"
...
function buildQueue(words: LinkingWord[], size: number): LinkingWord[] {
...
const CATEGORY_FILTERS: { key: LinkingWordCategory | "ALL"; label: string }[] = [
  { key: "ALL", label: "Toutes" },
  { key: "ADD", label: LINKING_WORD_CAT_LABELS.ADD },
  { key: "CONTRAST", label: LINKING_WORD_CAT_LABELS.CONTRAST },
  { key: "CAUSE", label: LINKING_WORD_CAT_LABELS.CAUSE },
  { key: "CONSEQ", label: LINKING_WORD_CAT_LABELS.CONSEQ },
  { key: "SEQ", label: LINKING_WORD_CAT_LABELS.SEQ },
  { key: "EXAMPLE", label: LINKING_WORD_CAT_LABELS.EXAMPLE },
]

export function LinkingWordsSession() {
  const [allWords, setAllWords] = useState<LinkingWord[]>(() =>
    LINKING_WORD_PHRASES.map((p) => ({ ...p, mastery: 0, wrong: 0, timesShown: 0 })),
  )
  const [loadingProgress, setLoadingProgress] = useState(true)
  const [categoryFilter, setCategoryFilter] = useState<LinkingWordCategory | "ALL">("ALL")
  const [seriesSize, setSeriesSize] = useState<number>(DEFAULT_SERIES_SIZE)
  const [queue, setQueue] = useState<LinkingWord[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        console.error("Erreur chargement progression (linking words):", e)
      } finally {
  ...
  const markShown = (wordId: string) => {
    setAllWords((prev) => {
      ...
      if (w) {
        ...
        }).catch((e) => console.error("Erreur sauvegarde progression (linking words):", e))
      }
      return updated
    })
  }
  ...
  const restart = (size: number = seriesSize, category: LinkingWordCategory | "ALL" = categoryFilter) => {
    setSeriesSize(size)
    ...
    try {
      const res = await fetch("/api/coach", {
        ...
        body: JSON.stringify({
          ...
          userAnswer: answer,
          context: LINKING_WORD_CAT_CONTEXT[current.cat],
  ...
  if (!current) {
    return (
      <div>
        {categorySelector}
        <div className={styles.muted}>Aucun connecteur dans cette catégorie.</div>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className={styles.muted} style={{ marginBottom: 10, fontSize: 13 }}>
        Les connecteurs logiques (cause, conséquence, opposition...) cassent presque autant que les chiffres à
        l'oral — entraîne-toi à les placer dans une vraie phrase, pas isolés.
      </div>
      {categorySelector}
      {seriesSizeSelector}
      <div className={styles.badge}>{LINKING_WORD_CAT_LABELS[current.cat]}</div>
      <div className={styles.cardStage}>
