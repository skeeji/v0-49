...
import { NUMBER_CAT_LABELS, NUMBER_CAT_CONTEXT, NUMBER_PHRASES, type NumberCategory, type NumberPhrase } from "../numbers-data"
import { useSpeechRecognition } from "../hooks/useSpeech"
import { HoverWord } from "./HoverWord"
import { PronunciationCheck } from "./PronunciationCheck"
import type { CoachResponse } from "../types"

// Mode "Chiffres" — même mécanique que SurvivalPhrasesSession (numbers-data.ts
// plutôt que survival-data.ts) : les chiffres/mesures sont ce qui casse le plus
// à l'oral sous stress, donc un entraînement dédié plutôt que noyés dans le
// vocabulaire général. Composant volontairement autonome, comme les Phrases
// utiles, pour ne rien changer au fonctionnement des Flashcards existantes.

interface NumberWord extends NumberPhrase {
  mastery: number
  wrong: number
  timesShown?: number
}
...
const SERIES_SIZES = [10, 20, 38] as const
...
// Flashcards et Phrases utiles) — sans collision possible avec leurs ids "w1"/"sp-".
const ID_PREFIX = "num-"
...
function buildQueue(words: NumberWord[], size: number): NumberWord[] {
...
const CATEGORY_FILTERS: { key: NumberCategory | "ALL"; label: string }[] = [
  { key: "ALL", label: "Toutes" },
  { key: "LUX", label: NUMBER_CAT_LABELS.LUX },
  { key: "PCT", label: NUMBER_CAT_LABELS.PCT },
  { key: "DIM", label: NUMBER_CAT_LABELS.DIM },
  { key: "TIME", label: NUMBER_CAT_LABELS.TIME },
  { key: "QTY", label: NUMBER_CAT_LABELS.QTY },
]

export function NumbersSession() {
  const [allWords, setAllWords] = useState<NumberWord[]>(() =>
    NUMBER_PHRASES.map((p) => ({ ...p, mastery: 0, wrong: 0, timesShown: 0 })),
  )
  ...
  const [queue, setQueue] = useState<NumberWord[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        console.error("Erreur chargement progression (chiffres):", e)
      } finally {
  ...
  const markShown = (wordId: string) => {
    setAllWords((prev) => {
      ...
      if (w) {
        ...
        }).catch((e) => console.error("Erreur sauvegarde progression (chiffres):", e))
      }
      return updated
    })
  }
  ...
  const restart = (size: number = seriesSize, category: NumberCategory | "ALL" = categoryFilter) => {
    setSeriesSize(size)
    ...
    try {
      const res = await fetch("/api/coach", {
        ...
        body: JSON.stringify({
          ...
          userAnswer: answer,
          context: NUMBER_CAT_CONTEXT[current.cat],
  ...
  if (!current) {
    return (
      <div>
        {categorySelector}
        <div className={styles.muted}>Aucun chiffre dans cette catégorie.</div>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className={styles.muted} style={{ marginBottom: 10, fontSize: 13 }}>
        Les chiffres sont ce qui casse le plus vite à l'oral sous stress — entraîne-toi à les dire dans une vraie
        phrase, pas isolés.
      </div>
      {categorySelector}
      {seriesSizeSelector}
      <div className={styles.badge}>{NUMBER_CAT_LABELS[current.cat]}</div>
      <div className={styles.cardStage}>
