...
export default function LuminaireDetailPage() {
  ...
  // --- CONSERVÉ : Cette partie est correcte ---
  useEffect(() => {
    if (!params.id) return;
  ...
  // --- FIN DES PARTIES CONSERVÉES ---


  // --- DÉBUT DE LA CORRECTION POUR LE PDF ---
  const generatePDF = async () => {
    ...
    try {
      ...
      // Étape 1: Charger l'image depuis l'API et la convertir en base64
      ...
      const addInfoLine = (label: string, value: string | string[], isMultiline = false) => {
        ...
        yPosition += (textLines.length * 7) + 2; // Ajuster l'espacement
      }
  ...
  // --- FIN DE LA CORRECTION POUR LE PDF ---
  ...
  return (
    // CONSERVÉ : TOUT VOTRE JSX EST IDENTIQUE
