...
export default function LuminairesPage() {
  ...
  // Charger les luminaires depuis l'API MongoDB avec scroll infini
        ...
        if (response.ok) {
          ...
          if (data.success && data.luminaires) {
            ...
            if (resetPage || currentPage === 1) {
              ...
              // CORRECTION: Ajouter les nouveaux luminaires à la liste existante (scroll infini)
  ...
  // CORRECTION: Scroll infini amélioré
  useEffect(() => {
    const handleScroll = () => {
    ...
    try {
      const response = await fetch("/api/luminaires", {
        ...
        body: JSON.stringify({
          ...
          // CORRECTION: ne pas mettre 2025 par défaut, laisser vide si pas d'année
          annee: newLuminaire.year ? Number.parseInt(newLuminaire.year) : null,
    ...
    try {
      const response = await fetch(`/api/luminaires/${id}`, {
        ...
        body: JSON.stringify({
          ...
          annee: updates.year ? Number.parseInt(updates.year) : null,
