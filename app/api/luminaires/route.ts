...
// FORMATEUR DE DONNÉES UNIQUE POUR LA COHÉRENCE PARTOUT
const formatLuminaire = (luminaire: any) => {
  ...
  return {
    ...
    materiaux: Array.isArray(luminaire.materiaux) ? luminaire.materiaux : [], // Garantit un tableau, évite le crash .map
    image: luminaire.images?.[0] ? `/api/images/filename/${luminaire.images[0]}` : null,
  }
}

// FONCTION GET UNIFIÉE (LISTE + DÉTAIL + SIMILAIRES)
export async function GET(request: NextRequest) {
  try {
    ...
    // CAS : PAGE GALERIE (LISTE) - CORRECTION SLIDER ET TRI
    const page = Number(searchParams.get("page") || "1")
    const limit = Number(searchParams.get("limit") || "50")
    const sliderModified = searchParams.get("sliderModified") === "true" // Le frontend enverra ce paramètre
...
// FONCTION POST "TOUT-EN-UN" (CRÉATION COMPLÈTE)
  ...
  try {
    ...
    if (designerName) {
      ...
      if (designerImage) {
        ...
        // Utilisation du nom de champ `imagedesigner` trouvé dans le CSV
    ...
    if (luminaireImage) {
      ...
      await luminairesCollection.updateOne({ _id: newLuminaireId }, { $set: { images: [filename] } })
    }

    return NextResponse.json({ success: true, id: newLuminaireId })
  } catch (error: any) {
