...
export async function GET() {
  try {
    ...
    // D'abord, regarder comment les images sont référencées dans les luminaires
    const luminairesCollection = db.collection("luminaires")
    const luminaires = await luminairesCollection.find({}).limit(10).toArray()

    console.log("🔍 Échantillon de luminaires pour comprendre le stockage:")
    luminaires.forEach((lum, index) => {
      if (index < 3) {
        console.log(`Luminaire ${index + 1}:`, {
          filename: lum.filename || lum["Nom du fichier"],
          images: lum.images,
          _id: lum._id,
        })
      }
    ...
    // Essayer de trouver les images via GridFS avec le bucket "images"
    ...
    // Si pas de fichiers dans GridFS, essayer de les récupérer via l'API existante
    if (files.length === 0) {
      console.log("🔄 Tentative de récupération via les filenames des luminaires...")

      const luminairesWithFiles = await luminairesCollection
        .find({
          $or: [{ filename: { $exists: true, $ne: "" } }, { "Nom du fichier": { $exists: true, $ne: "" } }],
        })
        .toArray()

      console.log(`📋 ${luminairesWithFiles.length} luminaires avec des fichiers trouvés`)

      const fileData: Array<{ name: string; data: Buffer; crc32: number }> = []

      // Pour chaque luminaire avec un fichier, essayer de récupérer l'image
      for (const luminaire of luminairesWithFiles.slice(0, 50)) {
        // Limiter à 50 pour éviter les timeouts
        const filename = luminaire.filename || luminaire["Nom du fichier"]
        if (!filename) continue

        try {
          console.log(`📁 Tentative de récupération: ${filename}`)

          // Essayer de récupérer via l'API existante
          const imageResponse = await fetch(
            `${process.env.NEXTAUTH_URL || "http://localhost:3000"}/api/images/filename/${filename}`,
          )
          ...
          if (imageResponse.ok) {
            ...
            if (imageBuffer.length > 0) {
              ...
              console.log(`✅ Image récupérée: ${filename} (${imageBuffer.length} bytes)`)
            }
          } else {
            console.log(`⚠️ Image non trouvée via API: ${filename}`)
          }
        } catch (fetchError) {
          console.error(`❌ Erreur récupération ${filename}:`, fetchError)
        }
      }

      if (fileData.length === 0) {
        return NextResponse.json({ error: "Aucune image n'a pu être récupérée" }, { status: 404 })
      }

      // Créer le ZIP avec les images récupérées
    ...
    // Si on a des fichiers GridFS, les traiter normalement
    const fileData: Array<{ name: string; data: Buffer; crc32: number }> = []

    for (const file of files) {
      try {
        console.log(`📁 Traitement GridFS: ${file.filename}`)
        ...
        console.log(`✅ Fichier GridFS traité: ${safeFilename} (${buffer.length} bytes)`)
      } catch (fileError) {
        console.error(`❌ Erreur avec le fichier GridFS ${file.filename}:`, fileError)
      }
    }
