...
        const imageResponse = await fetch("/api/upload/images", { method: "POST", body: imageFormData })
        const imageData = await imageResponse.json()
        if (!imageData.success) throw new Error(`Upload images: ${imageData.error}`)
...
        const designerImageResponse = await fetch("/api/upload/images", { method: "POST", body: designerFormData })
...
      }
      ...
      // CORRECTION : On ne construit qu'un seul objet de données propre et standardisé.
      const luminaireData = {
...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Le reste du JSX du formulaire reste inchangé */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="nom">Nom du luminaire *</Label>
              <Input id="nom" value={formData.nom} onChange={(e) => handleInputChange("nom", e.target.value)} placeholder="Nom du luminaire" required />
            </div>
            <div>
              <Label htmlFor="designer">Designer</Label>
              <Input id="designer" value={formData.designer} onChange={(e) => handleInputChange("designer", e.target.value)} placeholder="Nom du designer" />
            </div>
            <div>
              <Label htmlFor="annee">Année</Label>
              <Input id="annee" type="text" value={formData.annee} onChange={(e) => handleInputChange("annee", e.target.value)} placeholder="Année de création" />
            </div>
            <div>
              <Label htmlFor="periode">Période/Spécialité</Label>
              <Input id="periode" value={formData.periode} onChange={(e) => handleInputChange("periode", e.target.value)} placeholder="Période ou spécialité" />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="description">Description</Label>
              <Textarea id="description" value={formData.description} onChange={(e) => handleInputChange("description", e.target.value)} placeholder="Description du luminaire" rows={3} />
            </div>
            <div>
              <Label htmlFor="collaboration">Collaboration / Œuvre</Label>
              <Textarea id="collaboration" value={formData.collaboration} onChange={(e) => handleInputChange("collaboration", e.target.value)} placeholder="Informations sur la collaboration ou l'œuvre" rows={3} />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
...
                <SelectTrigger><SelectValue placeholder="Le luminaire est-il signé ?" /></SelectTrigger>
                <SelectContent>
...
              <Input id="editeur" value={formData.editeur} onChange={(e) => handleInputChange("editeur", e.target.value)} placeholder="Éditeur ou fabricant" />
            </div>
            <div>
              <Label htmlFor="dimensions">Dimensions</Label>
              <Input id="dimensions" value={formData.dimensions} onChange={(e) => handleInputChange("dimensions", e.target.value)} placeholder="ex: H 50cm x L 30cm x P 20cm" />
            </div>
            <div>
              <Label htmlFor="estimation">Prix / Estimation</Label>
              <Input id="estimation" value={formData.estimation} onChange={(e) => handleInputChange("estimation", e.target.value)} placeholder="Prix ou estimation" />
            </div>
          </div>
          <div>
            <Label>Matériaux</Label>
            <div className="flex gap-2 mb-2">
              <Input value={materialInput} onChange={(e) => setMaterialInput(e.target.value)} placeholder="Ajouter un matériau" onKeyPress={(e) => e.key === "Enter" && (e.preventDefault(), addMaterial())} />
              <Button type="button" onClick={addMaterial} variant="outline">Ajouter</Button>
            </div>
...
            <div className="flex gap-2 mb-2">
              <Input value={colorInput} onChange={(e) => setColorInput(e.target.value)} placeholder="Ajouter une couleur" onKeyPress={(e) => e.key === "Enter" && (e.preventDefault(), addColor())} />
              <Button type="button" onClick={addColor} variant="outline">Ajouter</Button>
            </div>
            <div className="flex flex-wrap gap-2">
...
                <Button type="button" onClick={() => fileInputRef.current?.click()} variant="outline"><Upload className="w-4 h-4 mr-2" /> Sélectionner des images</Button>
...
                        <Button type="button" size="sm" variant="ghost" onClick={() => removeImage(index)}><X className="w-4 h-4" /></Button>
                      </div>
                    ))}
                  </div>
                </div>
...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6">
              <input ref={designerImageInputRef} type="file" accept="image/*" onChange={handleDesignerImageUpload} className="hidden" />
              <div className="text-center">
                <ImageIcon className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                <Button type="button" onClick={() => designerImageInputRef.current?.click()} variant="outline"><Upload className="w-4 h-4 mr-2" /> Sélectionner l'image du designer</Button>
...
                    <Button type="button" size="sm" variant="ghost" onClick={removeDesignerImage}><X className="w-4 h-4" /></Button>
                  </div>
                </div>
              )}
            </div>
...
            <Button type="submit" disabled={isSubmitting} style={{ backgroundColor: "#f2d895", color: "#000" }} className="hover:opacity-90">
